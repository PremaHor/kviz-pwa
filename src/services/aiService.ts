...
/** Anglický vizuální motiv pro ukázkový kvíz (sladěno s volbou tématu). */
...
function buildUserPrompt(config: QuizConfiguration): string {
...
function quizResponseJsonSchema(questionCount: number): Record<string, unknown> {
  const qItem = {
    type: 'object',
    properties: {
...
function maxOutputTokensForQuiz(questionCount: number): number {
...
async function generateQuizWithGemini(
  ...
  if (!apiKey) {
    throw new Error('Chybí proměnná prostředí VITE_GEMINI_API_KEY.')
  }
  ...
  const body = {
    ...
    generationConfig: {
      ...
      // U modelů s „thinking“ jinak ubývají tokeny na samotný JSON (riziko MAX_TOKENS u 25–35 otázek).
  ...
  if (finishReason === 'SAFETY') {
    throw new Error('Gemini zablokovala odpověď (bezpečnostní filtry). Zkus upravit téma nebo kategorii.')
  }
  ...
  const quiz = parseGeneratedQuiz(parsed, questionCount)
  return enrichQuizWithMedia(quiz, config)
}

/** Offline / bez klíče: krátký mock se správným tématem v názvu. */
function generateQuizMock(config: QuizConfiguration): GeneratedQuiz {
  ...
  return {
    title: `${themeTitle[config.theme]} (ukázka bez API)`,
    questions: [
      {
        id: 'q1',
        questionText:
          'Toto je ukázková otázka bez připojené AI. Nastav VITE_GEMINI_API_KEY.',
        options: ['Ano, nastavím klíč', 'Ne', 'Možná', 'Nevím'],
        correctAnswerIndex: 0,
        mediaSearchHint: 'laptop computer programming',
        explanation:
          'Pro generování kvízu přes Google Gemini přidej do .env řádek VITE_GEMINI_API_KEY=… a restartuj dev server.',
      },
      {
        id: 'q2',
        questionText: 'Které téma jsi zvolil v průvodci?',
        options: [
          THEME_CS[config.theme],
          'Jiné téma',
          'Náhodné',
          'Žádné',
        ],
        ...
        explanation: `V konfiguraci je téma: ${THEME_CS[config.theme]}.`,
      },
    ],
  }
}
...
/**
 * Vygeneruje kvíz podle konfigurace (téma, kategorie, skupina, handicapy).
 * S `VITE_GEMINI_API_KEY` volá Google Gemini (`gemini-2.5-flash` nebo `VITE_GEMINI_MODEL`).
 * Bez klíče vrátí malý mock a v konzoli zobrazí upozornění.
 */
export async function generateQuiz(
  ...
  if (!apiKey) {
    console.warn(
      '[kvíz] VITE_GEMINI_API_KEY není nastaven — používám ukázkový kvíz. Pro AI (Gemini 2.5 Flash) přidej klíč z Google AI Studio do .env.'
    )
    await new Promise((r) => setTimeout(r, 1200))
    const quiz = generateQuizMock(config)
    return enrichQuizWithMedia(quiz, config)
  }
  return generateQuizWithGemini(config)
}
